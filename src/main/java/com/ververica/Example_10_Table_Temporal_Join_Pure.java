    ...
    Table deduplicateTransactions =
        tableEnv.sqlQuery(
            "SELECT t_id, t_time, t_customer_id, t_amount\n"
                + "FROM (\n"
                + "   SELECT *, ROW_NUMBER() OVER (PARTITION BY t_id ORDER BY t_time) AS row_num\n"
                + "   FROM Transactions)\n"
                + "WHERE row_num = 1");
    ...
    tableEnv
        .executeSql(
            "SELECT t_time, c_rowtime, t_id, c_name, t_amount\n"
                + "FROM DeduplicateTransactions LEFT JOIN Customers FOR SYSTEM_TIME AS OF t_time ON c_id = t_customer_id")
        .print();
  }
}
